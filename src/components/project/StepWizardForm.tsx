  ...
  return (
    <div className="flex flex-col gap-10 w-full max-w-2xl mx-auto bg-white rounded-2xl py-12 px-6 sm:px-12">
        ...
        <div className="flex flex-col items-center justify-center mb-2">
          {profile.avatar && (
            <img
              src={profile.avatar}
              alt={profile.label}
              className="w-20 h-20 rounded-full mb-2 border-2 border-gray-200 object-cover"
            />
          )}
          <div className="text-xl font-bold text-gray-900 text-center">{profile.label}</div>
          {profile.description && (
            <div className="text-base text-gray-500 text-center mt-1 max-w-md">
              {profile.description}
            </div>
          )}
        </div>
      ...
      <div className="flex flex-col mb-6">
        ...
        {s.helperText && (
          <div className="ml-14 mt-2 text-lg font-semibold text-gray-800">{s.helperText}</div>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2 mb-8">
        ...
        {s.helperText && <p className="text-base text-gray-500 mt-2">{s.helperText}</p>}
      </div>
